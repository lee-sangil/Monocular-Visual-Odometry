...
void MVO::plot(const Eigen::MatrixXd& depthMap) const {
	...
	distance.copyTo(mvo(cv::Rect(img.cols+2*GAP,gt_dist.rows+2*GAP,distance.cols,distance.rows)));
	...
	cv::putText(mvo, "Depth", cv::Point(gt_dist.cols*0.5+img.cols+2*GAP,gt_dist.rows+GAP*1.7), cv::FONT_HERSHEY_DUPLEX, 0.5, cv::Scalar::all(255), 1, CV_AA);
	cv::imshow("MVO", mvo);
