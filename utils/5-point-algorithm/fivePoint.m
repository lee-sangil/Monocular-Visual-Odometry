function [eMatrix, num] = fivePoint(x1_, x2_)
% -------------------------------------------------------------------------
...
x2 = x2_(:, 1);
y2 = x2_(:, 2);
A = [x2.*x1, x2.*y1, x2, y2.*x1, y2.*y1, y2, x1, y1, ones(length(x1), 1)];
% Perform SVD
[~, ~, V] = svd(A);
...
x1 = t3t4(B23, B12) - t3t4(B13, B22);
x2 = t3t4(B13, B21) - t3t4(B23, B11);
p3 = t3t3(B11, B22) - t3t3(B12, B21);

% Univariate polynomial of order 10 with z as the variable
detB = t3t7(x1, B31) + t3t7(x2, B32) + t4t6(p3, B33);
