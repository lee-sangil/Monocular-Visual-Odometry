...
for zz=1:500 % the number of experiments
    seqseq = 1;
    startt = tic;
    
    for seq=1:155 % the number of sequence
        %% loading
        ...
        T_c = 100; % convergence test, noise free : 15, noise : 5
...
%         best_overlap = 0;
        ...
        final = motion_segmentation(y, T, T_i, T_c, T_r, alpha, lambda, c);

        fprintf('\n- excution time (sec) :');
        ex_time(zz) = toc(startt);
		fprintf('%.3f', ex_time(zz));
...
% %         error_rate' .* 100
    ...
    %     iter
        fprintf('\n- mean error rate : %.3f %%\t', mean(error_rate).*100);
        group_number(seqseq) = max(s);

        fprintf('\n- total error rate : %.3f %%\t', mean(mean(error_rate(:,1:end-1))).*100);
        fprintf('\n- 2 motion error rate : %.3f %%\t', mean(mean(error_rate(group_number == 2,1:end-1))).*100);
        fprintf('\n- 3 motion error rate : %.3f %%\t', mean(mean(error_rate(group_number == 3,1:end-1))).*100);
        fprintf('\n\n');
        seqseq = seqseq + 1;
    end
%save '131027'
